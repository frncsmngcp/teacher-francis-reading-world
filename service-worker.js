const CACHE_VERSION = "wonderquest-reading-world-pwa-v1.5.15-tab-title-13";
...
function isImmutableAssetURL(url) {
  try {
    ...
    // All Reading World files beneath /assets/ use generated/content-hashed
    // filenames. A changed asset receives a new URL, so the old response is
    // safe to reuse across releases.
    return parsed.origin === self.location.origin && parsed.pathname.includes('/assets/');
  } catch (_) {
    return false;
  }
}
...
async function migrateLegacyAssets() {
  ...
  for (const cacheName of keys) {
    ...
    if (!cacheName.startsWith('teacher-francis-reading-world-pwa-v')) continue;
...
self.addEventListener('activate', event => {
  event.waitUntil((async () => {
    ...
    for (const key of await caches.keys()) {
      if (key === CORE_CACHE || key === ASSET_CACHE) continue;
      if (key.startsWith('teacher-francis-reading-world-pwa-v')) await caches.delete(key);
    }
