const CACHE_VERSION = "teacher-francis-reading-world-pwa-v1.5.12";
  ...
  if (failed.length) {
    ...
    async function retryWorker() {
      while (true) {
        ...
        await cacheOne(job.cache, job.url, { fresh: job.fresh });
      }
    }
...
function requestFullLibraryPrecache() {
  if (!fullLibraryPrecachePromise) {
    fullLibraryPrecachePromise = precacheReadingWorld().finally(() => {
